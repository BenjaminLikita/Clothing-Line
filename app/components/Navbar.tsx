    ...
    <div className={`bg-transparent border-b ${border} shadow-sm ${color} bg-black ${opacity}`}>
        <div className='w-[90%] flex justify-between items-center m-auto md:p-5 py-3'> 
          ...
          <nav className={clsx('justify-between font-medium text-xl md:text-lg block md:flex bg-gray-500 bg-opacity-95 w-[60%] md:w-[55%] lg:w-[40%] text-gray-700 md:text-inherit h-full transition-all duration-500 md:bg-transparent fixed md:relative top-0 p-10 pl-5 pt-14 md:p-0', {
            ...
            />
            <Link href={"/"}className={`py-3 px-5 my-2 md:my-0 block md:inline-block lg:hover:scale-110 transition-all duration-700 ${pathname === '/' && 'active'}`}>Home</Link>
            <Link href={"/gallery"} className={`py-3 px-5 my-2 md:my-0 block md:inline-block lg:hover:scale-110 transition-all duration-700 ${pathname === '/gallery' && 'active'}`}>Gallery</Link>
            <Link href={"/contact"} className={`py-3 px-5 my-2 md:my-0 block md:inline-block lg:hover:scale-110 transition-all duration-700 ${pathname === '/contact' && 'active'}`}>Contact Us</Link>
          </nav>
